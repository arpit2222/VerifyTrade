...
>(({ className, children, error, label, id, ...props }, ref) => {
  ...
  return (
    <div className="w-full">
      {label && (
        <label htmlFor={triggerId} className="mb-1.5 block text-sm font-medium text-zinc-300">
          {label}
        </label>
      )}
      <SelectPrimitive.Trigger
        ...
        className={cn(
          `flex h-10 w-full items-center justify-between rounded-lg border
           bg-zinc-800/60 px-3 py-2 text-sm text-zinc-100
           placeholder:text-zinc-500 transition-colors
           focus:outline-none focus:ring-2 focus:ring-blue-500
           disabled:cursor-not-allowed disabled:opacity-50
           [&>span]:line-clamp-1`,
          error ? "border-red-600" : "border-zinc-700 hover:border-zinc-600",
          className
        )}
        aria-invalid={!!error}
        {...props}
    ...
    <SelectPrimitive.Content
      ...
      className={cn(
        `relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-lg border
         border-zinc-700 bg-zinc-900 text-zinc-100 shadow-lg
  ...
  <SelectPrimitive.Item
    ...
    className={cn(
      `relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2
       text-sm outline-none focus:bg-zinc-800 focus:text-zinc-100
