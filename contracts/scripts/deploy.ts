...
async function main() {
  ...
  // Write/update .env.local in both root and frontend/ for immediate use
  const isZeroG = network.name === "zeroGGalileo";
  const prefix  = isZeroG ? "NEXT_PUBLIC_ZG_" : "NEXT_PUBLIC_";

  const envContent = `# Auto-generated by deploy.ts at ${result.timestamp}
# Network: ${network.name} (${chainId})

${prefix}CHAIN_ID=${chainId}
${prefix}NETWORK_ENV=${isZeroG ? "0g-galileo" : network.name === "arbitrumSepolia" ? "testnet" : "local"}

${prefix}VERIFY_TRADE_ADDRESS=${vtAddress}
${prefix}VERIFIABLE_TRADE_EXECUTOR_ADDRESS=${vteAddress}
${prefix}FAIRNESS_PROOF_ADDRESS=${fpAddress}
${prefix}MEV_REGISTRY_ADDRESS=${mrAddress}
`;
  const envPath         = path.resolve(__dirname, "../../.env.local");
  const frontendEnvPath = path.resolve(__dirname, "../../frontend/.env.local");

  // Append 0G vars without overwriting existing Arbitrum vars
  if (isZeroG) {
    const existing = fs.existsSync(frontendEnvPath) ? fs.readFileSync(frontendEnvPath, "utf8") : "";
    const block    = `\n# 0G Galileo Testnet contracts\n${envContent}`;
    fs.writeFileSync(frontendEnvPath, existing.includes("ZG_CHAIN_ID") ? existing : existing + block);
    fs.writeFileSync(envPath, (fs.existsSync(envPath) ? fs.readFileSync(envPath, "utf8") : "") + block);
  } else {
  ...
  if (network.name === "zeroGGalileo") {
    console.log("  Verify on 0G ChainScan:");
    console.log(`    npx hardhat verify --network zeroGGalileo ${vteAddress}`);
    console.log(`    npx hardhat verify --network zeroGGalileo ${fpAddress}`);
    console.log(`    npx hardhat verify --network zeroGGalileo ${mrAddress}`);
    console.log(`    npx hardhat verify --network zeroGGalileo ${vtAddress} "${proofValidator}"`);
